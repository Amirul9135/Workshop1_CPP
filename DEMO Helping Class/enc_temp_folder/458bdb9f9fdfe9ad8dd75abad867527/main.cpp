...
void registerAccount() {
	...
	while (1) {
		if (showPassword) {
			...
			rgMenu.setValue(2, "SHOW");
			rgMenu.setValue(1, tmpPassword);
		}
		else {
			rgMenu.setValue(2, "HIDE");
			rgMenu.setValue(1, hideString(tmpPassword));
		}
		...
		case 4:
			...
			if (isNumeric(tmpinput) && tmpinput.length() == 4) {
				...
				rgMenu.setValue(4, to_string(newacc.yearOfBirth));
			}
			else {
...
Account profile(Account user) {
	...
	while (1) {
		...
		profileMenu.setValue(1, hideString(temp.getPassword()));
		...
		case 1:
			...
			if (user.MatcPasswordWith(tmpInput)) { 
